...
import static org.jzb.weixin.work.util.WorkConstant.USER_SIMPLE_LIST_URL_TPL;
 ...
 */
public class UserSimpleListRequest implements Callable<UserSimpleListResponse> {
    protected final AgentClient client;
    protected final long department_id;
    protected final boolean fetch_child;

    protected UserSimpleListRequest(AgentClient client, long department_id, boolean fetch_child) {
        this.client = client;
    ...
    @Override
    public UserSimpleListResponse call() throws Exception {
        ...
        final Request request = new Request.Builder()
                .url(J.strTpl(USER_SIMPLE_LIST_URL_TPL, map))
                .build();
        try (Response response = OK_CLIENT.newCall(request).execute()) {
            ...
            try (ResponseBody body = response.body();
                ...
                return new UserSimpleListResponse(client, node);
            }
        }
    }

    public static class Builder implements Callable<UserSimpleListResponse> {
        protected final AgentClient client;
        ...
        public UserSimpleListRequest build() {
            return new UserSimpleListRequest(client, department_id, fetch_child);
        }
